...
public class SwapNodes {
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<nodeCount ; i++) {
			...
			if (leftVal != -1) {
				curNode.left = new Node(leftVal);
				q.add(curNode.left);
			}

			if (rightVal != -1) {
				curNode.right = new Node(rightVal);
				q.add(curNode.right);
			}
		}
	...
	private static void swap(Node root, int curLevel, int level) {
		if (root.left == null && root.right == null) {
			return;
		}

		// swap
		if (curLevel >= level && curLevel % level == 0) {
			Node temp = root.left;
		...
		if (root.left != null)
			swap(root.left, curLevel + 1, level);
		if (root.right != null)
			swap(root.right, curLevel + 1, level);
	}
